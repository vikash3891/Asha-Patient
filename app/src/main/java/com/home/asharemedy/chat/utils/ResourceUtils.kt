...
private fun getCircleColor(@IntRange(from = RANDOM_COLOR_START_RANGE.toLong(), to = RANDOM_COLOR_END_RANGE.toLong())
    ...
    val colorId = AshaRemedyApp.getInstance().resources.getIdentifier(colorIdName, "color", AshaRemedyApp.getInstance().packageName)
    return AshaRemedyApp.getInstance().resources.getColor(colorId)
}

fun getGreyCircleDrawable(): Drawable {
    return getColoredCircleDrawable(AshaRemedyApp.getInstance().resources.getColor(R.color.color_grey))
}

private fun getColoredCircleDrawable(@ColorInt color: Int): Drawable {
    val drawable = AshaRemedyApp.getInstance().resources.getDrawable(R.drawable.shape_circle) as GradientDrawable
